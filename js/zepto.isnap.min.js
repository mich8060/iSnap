$(document).ready(function(){
	...
	$.fn.buildSnap = function(data) {
		//console.log(data);
		
		current = data.current[0];
		$('.snap-title').html(current.title);
		$('.snap-date').html(current.date);
		$('.snap-shot').attr('href',current.url);
		$('.snap-shot img').attr({
			src:current.image,
			title:current.title
		});
		...
		prev = data.prev[0];
		$('.previews a').eq(0).attr({
			href: prev.url,
			title: prev.title,
			data: prev.base_url,
			data_id: prev.id
		})
		$('.previews img').eq(0).attr({
			img: prev.image,
			title: prev.title
		});
	}
	...
	$('.previews a').on('click',function(e){
		...
		$.ajax({
			type: 'GET',
			url: 'http://api.isnap.me/snaps/id/'+$el.attr('data_id'),
			dataType: 'json',
			timeout: 5000,
			content:$('main'),
			success: function(data){
				$('main').animate({
					...
					history.pushState({}, $el.attr('title'), $el.attr('data'));
					$(this).buildSnap(data);
					$('main').animate({
						opacity:1
					},300);
